...
class Post:
    ...
    async def send_formatted_post_according_to_sub(self, sub: db.Sub):
        ...
        await self.send_formatted_post(
            ...
            embed_video=sub.embed_video if sub.embed_video != -100 else user.embed_video,
    ...
    async def send_formatted_post(self,
        ...
        :param embed_video: 1=YouTube/Vimeo link preview, 0=skip post
    ...
    async def test_format(self, user_id: int) -> bool:
        sub = await db.Sub.filter(feed__link=self.feed_link, user_id=user_id).get_or_none()
        if sub is None:
            user = await db.User.get_or_none(id=user_id)
            if user is None:
                result = await self.send_formatted_post(user_id=user_id, embed_video=1)
            else:
                result = await self.send_formatted_post(
                    user_id=user_id,
                    ...
                    embed_video=user.embed_video,
                    silent=not user.notify,
                    display_entry_tags=user.display_entry_tags,
                )
        else:
            result = await self.send_formatted_post_according_to_sub(sub=sub)
