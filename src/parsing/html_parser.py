...
from .medium import Video, Image, Media, Animation, Audio, UploadedImage, RedditVideo
from . import reddit_video, reddit_embed
...
class Parser:
    def __init__(self, html: str, feed_link: Optional[str] = None):
        ...
        self.embed_video_url: Optional[str] = None
    ...
    async def parse(self):
        ...
        if reddit_video_config.is_enabled():
            self._suppress_reddit_preview_when_video_present()
        self._suppress_reddit_preview_when_embed_video_present()
        self.parsed = True

    def _suppress_reddit_preview_when_video_present(self):
        """Use preview only as Video fallback, not as a separate photo in the album."""
        if not any(
                isinstance(m, RedditVideo) for m in self.media._media
        ):
            return
        for medium in self.media._media:
            if not isinstance(medium, Image):
                continue
            url = medium.original_urls[0] if medium.original_urls else ''
            if reddit_video.is_reddit_preview_image(url):
                medium.drop_silently = True

    def _suppress_reddit_preview_when_embed_video_present(self):
        """Send YouTube/Vimeo/etc. as link previews instead of blurry Reddit thumbnails."""
        if not self.embed_video_url:
            return
        for medium in self.media._media:
            if not isinstance(medium, Image):
                continue
    ...
    async def _parse_item(
        ...
        if tag == 'a':
            ...
            if reddit_embed.is_embed_video_url(href) and self.embed_video_url is None:
                self.embed_video_url = reddit_embed.canonical_embed_url(href)
            if reddit_video_config.is_enabled() and reddit_video.is_v_redd_it(href):
                if self.media.url_exists(href, loose=True) is None:
                    poster = next(
                        (
                            m.original_urls[0]
                            for m in self.media._media
                            if isinstance(m, Image) and m.original_urls
                            and reddit_video.is_reddit_preview_image(m.original_urls[0])
                        ),
                        None,
                    )
                    self.media.add(RedditVideo(
                        href,
                        type_fallback_urls=poster,
                    ))
        ...
        if tag == 'img':
            ...
            for _src in _multi_src:
                ...
                if full_url := reddit_video.full_image_url(_src):
                    _src = full_url
