...
logger = log.getLogger('RSSFunBot.medium')
...
class RedditVideo(Video):
    """
    Reddit v.redd.it: external URLs are never direct video files, so download HLS and upload locally.
    Poster/thumbnail is used only if download fails.
    """

    def __init__(self, urls: Union[str, list[str]], type_fallback_urls: Optional[Union[str, list[str]]] = None):
        super().__init__(urls, type_fallback_urls=type_fallback_urls)
        self.local_path: Optional[str] = None
        self.uploaded_file: Union[InputFile, InputFileBig, None] = None
        self._use_local_upload: bool = False
        self.video_duration: int = 0
        self.video_width: int = 0
        self.video_height: int = 0

    def telegramize(self) -> Optional[Union[InputMediaDocumentExternal, InputMediaUploadedDocument]]:
        if self.valid is None:
            raise RuntimeError('validate() must be called before telegramize()')
        if self._use_local_upload and self.uploaded_file:
        ...
        return super().telegramize()

    async def _try_download_and_prepare(self) -> bool:
        if not reddit_video_config.is_enabled() or self._use_local_upload:
            return self._use_local_upload
        v_url = next((u for u in self.original_urls if reddit_video.is_v_redd_it(u)), None)
        if not v_url:
            return False

        if not self.local_path:
            downloaded = await reddit_video.download_v_redd_it(v_url)
            if not downloaded:
                logger.info(f'Reddit video download failed, will use poster if available: {v_url}')
                return False
            self.local_path = downloaded.path
            self.video_duration = downloaded.duration
            self.video_width = downloaded.width
            self.video_height = downloaded.height
        ...
        if not self.uploaded_file:
            if not env.bot:
                logger.debug(f'Reddit video downloaded but bot unavailable for upload_file: {v_url}')
                return False
            try:
                with open(self.local_path, 'rb') as f:
                    self.uploaded_file = await env.bot.upload_file(
                        f, file_name=os.path.basename(self.local_path)
                    )
            except Exception as e:
                logger.info(f'Reddit video upload_file failed: {v_url}', exc_info=e)
                reddit_video.unlink_local(self.local_path)
                self.local_path = None
                self.uploaded_file = None
                return False

        self._use_local_upload = True
        self.valid = True
        self.need_type_fallback = False
        self.type_fallback_medium = None
        self.chosen_url = self.local_path
        logger.info(
            f'Reddit video ready for local send ({self.size / 1024 / 1024:.2f}MB): {v_url}'
        )
        return True

    def _cleanup_local_file(self) -> None:
        if self.local_path:
            reddit_video.unlink_local(self.local_path)
            self.local_path = None

    async def type_fallback(self, reason: Union[Exception, str] = None) -> bool:
        if await self._try_download_and_prepare():
            return True
        return await super().type_fallback(reason=reason)

    async def fallback(self, reason: Union[Exception, str] = None) -> bool:
        if self._use_local_upload:
            self.valid = False
            self._use_local_upload = False
            self.uploaded_file = None
            self._cleanup_local_file()
            return await super().fallback(reason=reason)
        if await self._try_download_and_prepare():
            return True
        return await super().type_fallback(reason=reason)
    ...
    async def upload(self, chat_id: int, force_upload: bool = False) \
            -> tuple[Optional[TypeMessageMedia], Optional[TypeMedium]]:
        if not self._use_local_upload:
            await self._try_download_and_prepare()
        try:
            return await super().upload(chat_id, force_upload=force_upload)
        finally:
            if self.local_path:
                self._cleanup_local_file()
