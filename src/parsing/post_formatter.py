...
from .medium import Media, AbstractMedium, Image, Video, Audio, File, Animation, RedditVideo
...
class PostFormatter:
    ...
    async def get_formatted_post(self,
                                 ...
                                 style: int = 0,
                                 display_media: int = 0,
                                 embed_video: int = 1) -> Optional[tuple[str, bool, bool]]:
        """
        Get formatted post.
        ...
        :param style: 0=classic, 1=flowerss
        ...
        assert style in {CLASSIC, FLOWERSS}

        custom_sub_title = sub_title
        ...
        if self.embed_video_url and embed_video == 0:
            return None
        ...
        if send_mode == FORCE_MESSAGE:
            ...
            normal_msg_post = self.generate_formatted_post(sub_title=sub_title,
                                                           custom_sub_title=custom_sub_title,
                                                           tags=tags,
        ...
        media_count = await self.media.estimate_message_counts() if self.media else 0
        need_media = (
                media_count > 0
                and (
        ...
        if self.embed_video_url:
            need_link_preview = True
        ...
        if (
                (
                        ...
                        and not self.embed_video_url
                )  # ONLY_MEDIA_NO_CONTENT but no media
                or
                (
                        not self.parsed_html and not need_media and not self.embed_video_url
    ...
    def get_post_header_and_footer(self,
                                   ...
                                   custom_sub_title: Optional[str] = None) -> tuple[str, str]:
        # Classic style:
        # {title}
        # {hashtag}  (* optional)
        #
        ...
        custom_title_html = Bold(custom_sub_title).get_html() if custom_sub_title else None
        ...
        if message_style == FLOWERSS_STYLE:
            # ---- feed title (source line only; custom title goes below hashtags) ----
    ...
    def generate_formatted_post(self,
                                ...
                                message_type: TypeMessageType,
                                message_style: TypeMessageStyle,
                                custom_sub_title: Optional[str] = None) -> str:
    ...
    async def parse_html(self):
        ...
        self.embed_video_url = parsed.parser.embed_video_url
        ...
        if self.enclosures:
            ...
            for enclosure in self.enclosures:
                ...
                if not utils.isAbsoluteHttpLink(enclosure.url):
                    ...
                    if reddit_video.is_v_redd_it(enclosure.url):
                        medium = RedditVideo(enclosure.url, type_fallback_urls=enclosure.thumbnail)
                    else:
                        medium = Video(enclosure.url, type_fallback_urls=enclosure.thumbnail)
                elif enclosure.type.startswith('image'):
                    image_url = reddit_video.full_image_url(enclosure.url) or enclosure.url
