...
logger = log.getLogger('RSSFunBot.tgraph')
        ...
        if not self._fc_lock.locked():  # if not already blocking
            async with self._fc_lock:  # block any other sending tries
                ...
                if retry_after >= 60:
                    ...
                    await self.create_account(short_name='RSSFunBot', author_name='RSSFunBot',
                                              author_url='https://github.com/mathewskdaniel/RSSFunBot')
        ...
        for token in self.tokens:
            ...
            try:
                if len(token) != 60:  # must be an invalid token
                    ...
                    await account.create_account(short_name='RSSFunBot', author_name='RSSFunBot',
                                                 author_url='https://github.com/mathewskdaniel/RSSFunBot')
...
class TelegraphIfy:
    ...
    async def generate_page(self):
        ...
        if self.feed_title:
            ...
            self.telegraph_author = 'RSSFunBot'
            self.telegraph_author_url = 'https://github.com/mathewskdaniel/RSSFunBot'

        self.telegraph_title = self.title or 'RSSFunBot'
        self.telegraph_html_content = (soup.decode() +
                                       '<p>'
                                       '<a href="https://github.com/mathewskdaniel/RSSFunBot">RSSFunBot</a>. '
