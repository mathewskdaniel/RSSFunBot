...
logger = log.getLogger('RSSFunBot.message')
    ...
    def __init__(self,
        ...
        if media_type == VIDEO:
            ...
            if isinstance(media, types.MessageMediaDocument) and media.document:
                video_attrs = tuple(
                    attr for attr in media.document.attributes
                    if isinstance(attr, types.DocumentAttributeVideo)
                )
            self.attributes = video_attrs or (types.DocumentAttributeVideo(0, 0, 0, supports_streaming=True),)
        elif media_type == ANIMATION:
            self.attributes = (types.DocumentAttributeAnimated(),)
        else:
