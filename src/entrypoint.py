...
# log
logger = log.getLogger('RSSFunBot')
...
async def pre():
    ...
    bot.add_event_handler(command.reddit_video_settings.cmd_reddit_video,
                          events.NewMessage(pattern=construct_command_matcher('/reddit_video')))
    ...
    bot.add_event_handler(command.sub.callback_list_toggle_sub,
                          events.CallbackQuery(pattern=rf'^list_toggle=\d+(\|\d+)?{callback_target_matcher}$'))
    ...
    bot.add_event_handler(command.reddit_video_settings.callback_reddit_video,
                          events.CallbackQuery(pattern=r'^reddit_video=.+$'))
...
def main():
    ...
    try:
        ...
        logger.info(
            f"RSSFunBot ({', '.join(env.VERSION.split())}) started!\n"
