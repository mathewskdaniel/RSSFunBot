...
async def cmd_test(
    ...
    try:
        ...
        summary = f'Test finished: sent {sent}/{len(entries_to_send)} post(s).'
        if skipped:
            summary += f' {skipped} skipped (empty or filtered).'
        if errors:
...
async def __send(chat_id, entry, feed_title, link) -> bool:
    ...
    return await post.test_format(chat_id)
