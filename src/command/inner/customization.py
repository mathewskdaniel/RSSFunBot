...
async def get_customization_buttons(sub_or_user: Union[db.Sub, db.User],
    ...
    if is_user:
        interval_d = length_limit_d = notify_d = send_mode_d = link_preview_d = display_media_d = display_author_d = \
            display_via_d = display_title_d = display_entry_tags_d = style_d = embed_video_d = False
        all_default = None
    else:
        ...
        embed_video_d = sub_or_user.embed_video == -100
    ...
    embed_video = sub_or_user.user.embed_video if embed_video_d else sub_or_user.embed_video
    buttons = (
        (
            Button.inline(
        ...
        (
            Button.inline(
                f"{i18n[lang]['embed_video']}: "
                + (FALLBACK_TO_USER_DEFAULT_EMOJI if embed_video_d else '')
                + i18n[lang][f'embed_video_{embed_video}'],
                data=(
                    f'set_default=embed_video{tail}'
                    if is_user
                    else f'set={sub_or_user.id},embed_video|{page}{tail}'
                ),
            ),
        ),
        (
