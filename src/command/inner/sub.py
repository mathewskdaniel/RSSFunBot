...
from .utils import update_interval, list_sub, filter_urls, logger, escape_html, sub_list_label, \
...
async def subs(user_id: int,
    ...
    success_msg = (
            ...
            + '\n'.join(f'<a href="{sub_d["sub"].feed.link}">'
                        f'{escape_html(sub_list_label(sub_d["sub"]))}</a>'
