...
logger = log.getLogger('RSSFunBot.command')
...
def sub_list_label(sub: db.Sub) -> str:
    """Subscription name for pickers and lists; not the post-only custom title (sub.title)."""
    return sub.feed.title


def trim_button_label(text: str, max_len: int = 30) -> str:
    text = text.strip()
    if len(text) <= max_len:
        return text
...
async def get_sub_choosing_buttons(user_id: int,
    ...
    buttons_to_arrange = tuple(Button.inline(sub_list_label(_sub),
...
async def get_sub_list_buttons(user_id: int,
                               page_number: int,
                               get_page_callback: str,
                               lang: Optional[str] = None,
                               rows: int = 10,
                               ...
                               *args, **kwargs) -> Optional[tuple[tuple[KeyboardButtonCallback, ...], ...]]:
    """
    Interactive subscription list: per sub, a status toggle button and a settings button.
    """
    if page_number <= 0:
        raise IndexError('Page number must be positive.')

    page_number, page_count, page, sub_count = await get_sub_list_by_page(
        user_id=user_id, page_number=page_number, size=rows, *args, **kwargs,
    )

    if page_count == 0:
        return None

    active_mark = i18n[lang]['list_status_active_mark']
    inactive_mark = i18n[lang]['list_status_inactive_mark']
    settings_label = i18n[lang]['list_settings_button']

    buttons_to_arrange = []
    for _sub in page:
        label = trim_button_label(sub_list_label(_sub))
        status_mark = active_mark if _sub.state == 1 else inactive_mark
        buttons_to_arrange.append(Button.inline(
            f'{status_mark} {label}',
            data=f'list_toggle={_sub.id}|{page_number}{tail}',
        ))
        buttons_to_arrange.append(Button.inline(
            settings_label,
            data=f'set={_sub.id}|{page_number}{tail}',
        ))

    buttons = arrange_grid(to_arrange=buttons_to_arrange, columns=2, rows=rows)
