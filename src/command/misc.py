...
async def cmd_or_callback_help(
    ...
    buttons = None
    if event.chat_id in env.MANAGER:
        buttons = (
            Button.inline(
                i18n[lang]['cmd_description_reddit_video'],
                data='reddit_video=open',
            ),
            Button.inline(
                i18n[lang]['cmd_description_set_default'],
                data='set_default',
            ),
        )
    respond_kwargs = dict(parse_mode='html', link_preview=False, buttons=buttons)
    await (
        event.respond(msg, **respond_kwargs)
        if isinstance(event, TypeEventMsg) or not hasattr(event, 'edit')
        else event.edit(msg, **respond_kwargs)
    )
