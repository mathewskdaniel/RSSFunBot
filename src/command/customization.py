...
async def callback_reset(
    ...
    sub.length_limit = sub.notify = sub.send_mode = sub.link_preview = sub.display_author = sub.display_media = \
        sub.display_title = sub.display_entry_tags = sub.display_via = sub.style = sub.embed_video = -100
    await sub.save()
...
async def callback_reset_all(
    ...
    await db.Sub.bulk_update(
        ...
        (
            ...
            'display_title', 'display_entry_tags', 'display_via', 'style', 'embed_video',
        )
    )
    for task in tasks:
