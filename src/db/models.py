...
class User(Model, Base):
    ...
    display_title = fields.SmallIntField(default=1)
    display_entry_tags = fields.SmallIntField(default=-1)
    style = fields.SmallIntField(default=1)
    display_media = fields.SmallIntField(default=1)
    embed_video = fields.SmallIntField(
        default=1,
        description='YouTube/Vimeo etc. from Reddit: 1=link preview, 0=skip post',
    )
...
class Sub(Model, Base):
    ...
    title = fields.CharField(max_length=1024, null=True, description='Custom title shown in posts; not used in sub lists')
    ...
    embed_video = fields.SmallIntField(default=-100)
