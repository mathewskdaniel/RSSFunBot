...
logger = colorlog.getLogger('RSSFunBot.env')

# ----- determine the environment -----
user_home = os.path.expanduser('~')
self_path = os.path.dirname(__file__)
self_module_name = os.path.basename(self_path)
cli_entry = sys.argv[0]  # expect `-m` or `/path/to/rssfunbot.py`
is_self_run_as_a_whole_package = cli_entry.endswith('rssfunbot.py')
...
if custom_config_path:
    ...
    config_folder_path = os.path.join(user_home, '.rssfunbot')
...
USER_AGENT: Final = os.environ.get('USER_AGENT') or (
    f'RSSFunBot/{__version__} RSS Reader (+https://github.com/mathewskdaniel/RSSFunBot)'
)
...
_img_relay_server = os.environ.get('IMG_RELAY_SERVER') or 'https://wsrv.nl/?url='
...
# ----- Reddit v.redd.it video (download on external-URL failure) -----
REDDIT_VIDEO_DOWNLOAD: Final = __bool_parser(os.environ.get('REDDIT_VIDEO_DOWNLOAD'), default_value=True)
_media_temp_dir = os.environ.get('MEDIA_TEMP_DIR') or os.path.join(config_folder_path, 'media-temp')
MEDIA_TEMP_DIR: Final = os.path.normpath(os.path.abspath(_media_temp_dir))
del _media_temp_dir
REDDIT_VIDEO_MAX_HEIGHT: Final = max(220, min(720, int(os.environ.get('REDDIT_VIDEO_MAX_HEIGHT') or 480)))
REDDIT_VIDEO_MAX_BYTES: Final = int(os.environ.get('REDDIT_VIDEO_MAX_BYTES') or 20971520)
REDDIT_VIDEO_DOWNLOAD_TIMEOUT: Final = max(30, int(os.environ.get('REDDIT_VIDEO_DOWNLOAD_TIMEOUT') or 120))
